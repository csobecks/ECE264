...
int main()
{
	...
	int ct=0;
	char buffer[100], ch;
	struct   anonApplication * listptr,  *pL;
...
//		Delete Application from list
			...
			if(pL!=NULL)
				...
				printf("there is no papplication wuth application ID=  %d\n",app_to_be_gone);
			break;
...
//completed by BK
		...
		case 'S':
//		Sort by university
			listptr=sortByUniversity(listptr);
			break;
...
void print_main_menu()
{
	...
	printf("\tS\t\tsort list By university\n");
